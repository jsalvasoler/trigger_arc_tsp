...
\subsection{Variations of the \gls{tsp}}
...
For the synthetic instances (RG), the best integer solution obtained after running the Gurobi \gls{mip} with a time limit of 1 minute and the default settings is used.
...
The effect of the parameter $\alpha$ on the performance of the RGC is explored.
The RGC is run for 10 trials on each instance and each value of $\alpha$, and the best solution found is taken.
...
The performance of the \gls{mip}-based biased construction heuristic is evaluated
...
All datasets show the same ranking pattern: \gls{mip} Bias consistently achieves the best performance, followed by \gls{mip} $\alpha$-Rand and \gls{mip} $\beta$-Rand, with RGC and SRC performing significantly worse.
...
Given these results, the construction heuristic used in the \gls{grasp} framework is \gls{mip} Bias.
...
When solving the competition instances during the competition, the full \gls{grasp} with the \gls{mip} Bias construction heuristic with parameters $\alpha = 0.1$ and $\beta = 3.0$ was used.
